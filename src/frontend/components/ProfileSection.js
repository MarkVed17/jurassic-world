...
import { FollowersUsersModal } from "./FollowersUsersModal";
import { FollowingUsersModal } from "./FollowingUsersModal";

const ProfileSection = ({ profile }) => {
  const [editProfileModal, setEditProfileModal] = useState(false);
  const [followingUsersModal, setFollowingUsersModal] = useState(false);
  const [followersUsersModal, setFollowersUsersModal] = useState(false);
  ...
  return (
    <>
        ...
        <div className="flex flex-col grow gap-2 font-medium dark:text-white">
          ...
          <div className="flex gap-4">
            <div
              ...
              onClick={() => setFollowingUsersModal(true)}
            >
            ...
            <div
              className="flex gap-1 hover:cursor-pointer"
              onClick={() => setFollowersUsersModal(true)}
            >
      ...
      {followingUsersModal && (
        <FollowingUsersModal
          setFollowingUsersModal={setFollowingUsersModal}
          following={following}
        />
      )}
      {followersUsersModal && (
        <FollowersUsersModal
          setFollowersUsersModal={setFollowersUsersModal}
          followers={followers}
        />
      )}
    </>
